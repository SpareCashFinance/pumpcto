...
export default function SplitFlapText({
  ...
  useEffect(() => {
    ...
    // Reset tiles when the phrase list changes; the animation loop owns later updates.
    // eslint-disable-next-line react-hooks/set-state-in-effect -- phrase reset must land before the flip timer starts
    setTiles(createTiles(firstPhrase));

    if (normalizedPhrases.length <= 1 || typeof window === "undefined" || !loop) {
      // #region agent log
      fetch("http://127.0.0.1:7447/ingest/7261716d-045c-4378-bc38-b41af16803cc", {
        ...
        body: JSON.stringify({
          ...
          runId: "post-fix",
          hypothesisId: "F",
          location: "SplitFlapText.tsx:skipCycle",
          message: "split flap cycle skipped",
          data: { phrases: normalizedPhrases.length, loop },
          timestamp: Date.now(),
        }),
      }).catch(() => {});
      // #endregion
    ...
    const animateTo = (targetPhrase: string) => {
      ...
      const plans = targetChars
        .map((targetChar, index): AnimationPlan | null => {
  ...
  return (
    ...
    >
      {tiles.map((tile, index) => (
        <span className="split-flap-text__tile" key={`${index}-${tile.tick}-${tile.current}`}>
